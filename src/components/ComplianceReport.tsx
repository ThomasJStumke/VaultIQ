  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 glass-card p-8">
        <div>
          <h2 className="text-3xl font-black text-white tracking-tighter">Governance Node</h2>
          <p className="text-slate-500 font-medium mt-1">Official institutional readiness and telemetry logs.</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button className="flex items-center gap-2 px-5 py-2.5 bg-white/5 border border-white/10 rounded-2xl text-xs font-black uppercase tracking-widest text-slate-300 hover:text-white hover:bg-white/10 transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="glass-card p-8 text-center relative overflow-hidden group">
            ...
            <p className="relative z-10 text-[10px] font-black text-slate-500 uppercase tracking-widest mb-6 italic text-gradient">Readiness Pulse</p>
            <div className="relative z-10 inline-flex items-center justify-center w-36 h-36 rounded-full border-8 border-white/5 relative mb-6">
               ...
               <span className="text-4xl font-black text-white tracking-tighter">88<span className="text-lg text-slate-500">%</span></span>
            </div>
            <p className="relative z-10 text-sm font-black text-white uppercase tracking-tight">Institutional Health</p>
            <p className="relative z-10 text-[10px] font-bold text-slate-500 mt-2 uppercase tracking-widest">Target: 95% Node sync</p>
          </div>
          ...
          <div className="glass-card p-8 bg-indigo-600 shadow-lg shadow-indigo-600/20 border-indigo-500 relative overflow-hidden group hover:scale-[1.02]">
            ...
            <Lock className="w-8 h-8 mb-6 text-white/50 group-hover:text-white transition-colors" />
            <h4 className="font-black text-white text-lg tracking-tight mb-3">Secure Exam Workflows</h4>
        ...
        <div className="lg:col-span-3 glass-card p-10">
          <h3 className="font-black text-white text-xl uppercase tracking-tighter mb-10 flex items-center gap-4">
            <div className="w-1.5 h-6 bg-indigo-500 rounded-full" />
            Compliance Telemetry Log
          </h3>
          <div className="space-y-10 relative before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-px before:bg-white/5">
            {[
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-4 mb-1">
                    <h4 className="text-sm font-black text-white tracking-tight">{log.title}</h4>
                    <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest bg-white/5 px-2 py-0.5 rounded border border-white/5">{log.user}</span>
                  </div>
                  <p className="text-sm text-slate-400 font-medium leading-relaxed max-w-2xl">{log.desc}</p>
                  <p className="text-[10px] font-black text-slate-600 mt-3 uppercase tracking-widest italic">{log.time}</p>
                </div>
              </motion.div>
            ))}
          </div>
