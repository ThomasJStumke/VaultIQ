  ...
  const handleExecuteBulkInjection = async () => {
    ...
    try {
      ...
      for (const rec of validRecords) {
        ...
        // Resolve Lecturer if they match display name or email in our staff directory
        const matchedStaff = staffList.find(s => 
          s.email?.toLowerCase() === rec.lecturerName?.toLowerCase() || 
          s.displayName?.toLowerCase() === rec.lecturerName?.toLowerCase()
        );
        const lecturerVal = matchedStaff 
          ? [matchedStaff.displayName] 
          : (rec.lecturerName ? [rec.lecturerName] : []);
      ...
      setSuccessMsg(`Bulk uploaded and registered ${insertedCount} new academic modules with allocations!`);
      setBulkCSVText('');
      setParsedModules([]);
  ...
  if (loading) {
    return (
      <div className="h-64 flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Loading topology maps...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
          ...
          {profile?.role === 'FACULTY_ADMIN' && (
            <>
              <button 
                ...
                className="flex items-center gap-2 bg-indigo-600 text-white px-5 py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-indigo-500 transition shadow-lg shadow-indigo-600/15 active:scale-95 border border-white/5"
              >
              ...
              <button 
                ...
                className="flex items-center gap-2 bg-slate-800 text-slate-200 hover:text-white px-5 py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-700 transition border border-white/10 active:scale-95"
              >
      ...
      <AnimatePresence>
        {isAddingNewSyllabusModule && (
          ...
          >
            <div className="glass-card p-6 md:p-8 bg-slate-900 border border-indigo-500/20 rounded-2xl relative">
              ...
              <form onSubmit={handleCreateSyllabusModule} className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Module Code *</label>
                  <input 
                    type="text" 
                    placeholder="e.g. PRG311"
                    className="w-full px-4 py-3 bg-slate-800 border border-white/10 rounded-xl text-white font-semibold uppercase focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                    value={syllabusCode}
                ...
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Module Name *</label>
                  <input 
                    type="text" 
                    placeholder="e.g. Advanced Technical Programming"
                    className="w-full px-4 py-3 bg-slate-800 border border-white/10 rounded-xl text-white font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                    value={syllabusName}
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Assessment Scheme</label>
                  <select 
                    className="w-full px-4 py-3 bg-slate-800 border border-white/10 rounded-xl text-white font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-950"
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Target Faculty</label>
                  <select 
                    className="w-full px-4 py-3 bg-slate-800 border border-white/10 rounded-xl text-white font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-950"
                ...
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Target Department</label>
                  <select 
                    className="w-full px-4 py-3 bg-slate-800 border border-white/10 rounded-xl text-white font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-950"
                ...
                <div className="flex items-end gap-3 md:col-span-1">
                  <button 
                    type="submit"
                    className="w-full py-3 px-5 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-[10px] uppercase tracking-widest rounded-xl transition cursor-pointer"
                  >
                  ...
                  <button 
                    ...
                    className="py-3 px-4 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white font-black text-[10px] uppercase tracking-widest rounded-xl transition"
                  >
                    Cancel
                  </button>
                </div>
      ...
      <AnimatePresence>
        {isBulkImporting && (
          <motion.div 
            ...
            className="overflow-hidden mb-8 animate-in slide-in-from-top-4 duration-300"
          >
            <div className="glass-card p-6 md:p-8 bg-slate-900 border border-indigo-500/20 rounded-2xl" id="bulk-module-importer-panel">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 pb-4 border-b border-indigo-500/10">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition active:scale-95 cursor-pointer"
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition active:scale-95 cursor-pointer"
              ...
              {/* Instructions Box */}
              <div className="p-4 bg-slate-850 border border-indigo-500/10 rounded-xl mb-6 text-xs text-slate-300 space-y-3">
                <div className="p-3 bg-amber-500/10 border border-amber-500/20 text-amber-400 rounded-lg text-xs font-bold uppercase tracking-wider flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-[10px] normal-case text-slate-300 mt-1 leading-relaxed">
                ...
                <p>
                  1. <strong className="text-white">Code</strong>: Unique Module Code (e.g. TAX402, SYS313).
                  <br />
                  2. <strong className="text-white">Name</strong>: Dynamic syllabus title.
                  <br />
                  3. <strong className="text-white">AssessmentMode</strong>: Choose from <code className="text-indigo-300 font-mono text-[10px]">EXAM_BASED</code>, <code className="text-indigo-300 font-mono text-[10px]">CONTINUOUS_ASSESSMENT</code>, or <code className="text-indigo-300 font-mono text-[10px]">PROJECT_BASED</code>.
                  <br />
                  4. <strong className="text-white">DepartmentCode</strong> (Optional): Target registry (e.g. <code className="font-mono text-emerald-400">AUD_TAX</code>, <code className="font-mono text-emerald-400">MGT_ACC</code>, <code className="font-mono text-emerald-400">FIN_ACC</code>, <code className="font-mono text-emerald-400">INF_TECH</code>, <code className="font-mono text-emerald-400">INF_SYS</code>, <code className="font-mono text-emerald-400">INF_ICM</code>). If empty, the dropdown below will apply.
                  <br />
                  5. <strong className="text-white">LecturerEmailOrName</strong> (Optional): Email or custom name of the staff member allocated to the module. If matches our registry email, profile link will be automated.
                  <br />
                  6. <strong className="text-white">ExitLevel</strong> (Optional): Enter <code className="font-mono text-amber-400">YES</code> or <code className="font-mono text-amber-400">NO</code>. Denotes if module is exit-level (requires both internal & external moderation). Defaults to NO.
                </p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Fallback Faculty (When empty in CSV)</label>
                  <select 
                    className="w-full bg-slate-850 p-3 text-xs font-black text-white border border-white/10 rounded-xl text-slate-950"
                    value={bulkFacultyId}
                    onChange={(e) => {
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Fallback Department (When empty in CSV)</label>
                  <select 
                    className="w-full bg-slate-850 p-3 text-xs font-black text-white border border-white/10 rounded-xl text-slate-950"
                    value={bulkDeptId}
              ...
              <div className="space-y-4">
                ...
                <div 
                  ...
                  className={cn(
                    ...
                    isDragging 
                      ...
                      : "border-white/10 hover:border-indigo-500/50 bg-white/[0.02] hover:bg-white/[0.04] text-slate-400 hover:text-slate-200"
                  )}
                ...
                <div className="space-y-2">
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest">Syllabus Import Data list (Paste Offline CSV Content below)</label>
                  <textarea 
              ...
              {parsedModules.length > 0 && (
                <div className="mt-6 space-y-3">
                  <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">Ready to Inject ({parsedModules.filter(r => r.status === 'VALID').length} Valid records)</h4>
                        ...
                        {parsedModules.map((rec, idx) => {
                          ...
                          return (
                            <tr key={idx} className={rec.status === 'VALID' ? 'hover:bg-white/[0.01]' : 'bg-rose-500/5'}>
                              <td className="px-4 py-2 font-mono text-white font-bold">{rec.code}</td>
                              <td className="px-4 py-2 text-slate-300">{rec.name}</td>
                              <td className="px-4 py-2 text-slate-500">{rec.assessmentMode}</td>
                              <td className="px-3 py-2 text-slate-400">
                                {resolvedDeptDetails ? (
                                  <span className="text-emerald-400 font-semibold">{resolvedDeptDetails.code}</span>
                                ) : (
                                  <span className="text-slate-500 italic">Fallback Dept</span>
                                )}
                              </td>
                              <td className="px-3 py-2 text-indigo-300 italic">
                                {rec.lecturerName || 'None'}
                              </td>
                              <td className="px-3 py-2">
                                {rec.isExitLevel ? (
                                  ...
                                  <span className="text-[9px] text-slate-500 font-medium">NO</span>
                                )}
                              </td>
                  ...
                  <div className="flex justify-end gap-3">
                    <button 
                      ...
                      className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black uppercase tracking-widest rounded-xl transition active:scale-95"
                    >
      ...
      <AnimatePresence>
        {isAddingModule && (
          ...
          >
            <div className="glass-card p-6 md:p-8 bg-slate-900 border border-indigo-500/20 rounded-2xl relative mb-8">
              <h3 className="text-lg font-black text-white tracking-tight mb-4 flex items-center gap-2">
              ...
              <form onSubmit={handleCreateOrUpdateModule} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Pre-assigned Module <span className="text-indigo-400">*</span></label>
                  {editingModuleId ? (
                    <div className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-slate-300 font-extrabold text-sm uppercase">
                      {newCode} - {newName}
                    </div>
                  ) : (
                    <select 
                    ...
                    >
                      ...
                      {currentDeptPresets.length === 0 && (
                        <option value="" disabled className="text-slate-400">No master templates configured for this dept</option>
                      )}
                    </select>
                  )}
                </div>
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Department Assignment</label>
                  <div className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-slate-400 font-bold text-xs uppercase select-none">
                ...
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Assigned Lecturer <span className="text-indigo-400">*</span></label>
                  <select 
                    className="w-full px-4 py-3 bg-slate-800 border border-white/10 rounded-xl text-white font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                ...
                <div className="md:col-span-1">
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Exit Level Flag</label>
                  <div className="flex items-center justify-between px-4 py-3 bg-white/5 border border-white/10 rounded-xl h-[46px] select-none">
                ...
                <div className="flex items-center justify-end gap-3 md:col-span-3 border-t border-white/5 pt-4 mt-2">
                  <button 
                    type="submit"
                    className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-[10px] uppercase tracking-widest rounded-xl transition-colors duration-150 active:scale-95 border border-white/5 cursor-pointer"
                  >
                  ...
                  <button 
                    ...
                    className="px-6 py-2.5 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white font-black text-[10px] uppercase tracking-widest rounded-xl transition cursor-pointer"
                  >
                    Cancel
                  </button>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="glass-card p-6">
            <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-1.5 border-b border-white/5 pb-3">
          ...
          <div className="glass-card p-5 border border-white/5 bg-gradient-to-br from-slate-950 to-slate-900">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center text-xs">
                ...
                <span className="text-slate-300 font-bold">{selectedFaculty?.short}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-500 font-bold uppercase tracking-wide">Dept.</span>
                <span className="text-slate-300 font-bold">{selectedDept?.name}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="glass-card p-6 md:p-8 relative overflow-hidden bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950">
            <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
              <Layers className="w-56 h-56 text-indigo-500" />
            </div>

            <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-1.5 pb-3 border-b border-white/5">
            ...
            <div className="flex flex-col md:flex-row items-center justify-around gap-4 md:gap-2">
              ...
              {/* Faculty Node */}
              <div className="w-full md:w-1/4 p-4 rounded-2xl bg-white/5 border border-white/10 flex flex-col items-center text-center">
                ...
                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">FACULTY BOUNDARY</p>
                <p className="text-xs font-black text-white tracking-tight uppercase">{selectedFaculty?.name || 'FID'}</p>
              </div>
              ...
              <div className="w-full md:w-1/4 p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/15 flex flex-col items-center text-center">
                ...
                <p className="text-xs font-black text-white tracking-tight uppercase">{selectedDept?.name || 'CS'}</p>
              </div>
          ...
          <div className="glass-card p-6 space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="relative w-full sm:w-64">
                ...
                <input 
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-white/5 border border-white/10 rounded-xl text-xs text-white uppercase focus:outline-none focus:ring-1 focus:ring-indigo-500 placeholder:text-slate-600"
                  value={searchTerm}
            ...
            {displayedModules.length === 0 ? (
              <div className="p-12 border-2 border-dashed border-white/5 rounded-2xl text-center space-y-3">
                <Compass className="w-12 h-12 text-slate-600 mx-auto" />
                <p className="text-slate-400 text-sm font-bold uppercase tracking-wider">No allocated modules found</p>
                ...
                {['HOD', 'DEPUTY_DEAN', 'EXECUTIVE_DEAN'].includes(profile?.role || '') && (
                  <button 
                    ...
                    className="mt-4 px-4 py-2 bg-white/5 border border-white/10 hover:border-white/20 hover:bg-white/10 text-white rounded-xl text-xs font-black uppercase tracking-widest transition"
                  >
                    Allocate first code
                  </button>
                )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {displayedModules.map((m) => (
                  <motion.div 
                    layout
                    key={m.id}
                    className="p-5 bg-white/[0.02] border border-white/5 rounded-2xl flex flex-col justify-between hover:bg-white/[0.04] transition group"
                  >
                    <div>
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-xs font-black text-indigo-400 tracking-wider">
                            {m.code.substring(0, 3)}
                          </div>
                          <div>
                            <span className="text-xs font-black text-white tracking-tight">{m.code}</span>
                            <h5 className="text-sm font-bold text-slate-300 mt-0.5 line-clamp-1">{m.name}</h5>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-white/5">
                        <div className="text-left">
                          <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Compliance Status</p>
                          <span className={cn(
                    ...
                    <div className="mt-4 pt-3 border-t border-white/5 flex justify-between items-center text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[9px] px-2 py-0.5 bg-white/5 rounded border border-white/5 text-slate-400 font-bold">
                          NQF Level 8
                        </span>
                      </div>
                    </div>
