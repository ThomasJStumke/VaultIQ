  ...
  if (isProcessing) {
    return (
      <div className="p-8 bg-indigo-600/5 border border-indigo-500/20 rounded-3xl flex flex-col items-center justify-center text-center">
        ...
        <h4 className="text-foreground font-black tracking-tight text-lg">AI Multi-Modal Analysis in Progress...</h4>
        <p className="text-subtle-foreground text-sm font-medium mt-2">Gemini is verifying signatures, module codes, and content integrity.</p>
        
        <div className="w-full max-w-xs mt-8 h-1.5 bg-surface-tint rounded-full overflow-hidden">
          <motion.div 
    ...
    >
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-4">
          <div className={cn("p-3 rounded-2xl bg-surface-tint-strong", config.color)}>
            <Icon className="w-6 h-6" />
          </div>
          <div>
            <h4 className={cn("font-black tracking-tight", config.color)}>AI Validation: {result.status}</h4>
            <p className="text-subtle-foreground text-xs font-bold uppercase tracking-widest mt-0.5">Confidence Score: {result.confidence}%</p>
          </div>
        </div>
        <div className="text-right">
          <div className="flex items-center gap-2 text-xs font-black text-foreground/40 uppercase tracking-tighter">
      ...
      <div className="space-y-3 bg-surface-sunken p-4 rounded-2xl border border-border-subtle">
        <h5 className="text-[10px] font-black text-subtle-foreground uppercase tracking-widest flex items-center gap-2">
...
function AnalysisPill({ label, active, icon: Icon }: any) {
  return (
    <div className={cn(
      ...
      active ? "bg-emerald-500/5 border-emerald-500/20 text-emerald-500" : "bg-surface-tint border-border text-subtle-foreground grayscale opacity-50"
    )}>
