...
import { subscribeToModules, subscribeToEvidence, uploadEvidenceMetadata, updateModuleCompliance } from '../services/firebaseService';
            ...
            {isReportCenterOpen && (
              ...
              >
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="lg:col-span-8 p-6 bg-slate-900/40 border border-white/5 rounded-3xl flex flex-col justify-between gap-6">
                    <div className="space-y-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-sm font-black text-white tracking-tight uppercase">AI Report Scribe & Butler Assistant</h4>
                      ...
                      <form onSubmit={handleAIScribeQuery} className="flex gap-2.5">
                        <input 
                          ...
                          className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs text-white placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 transition font-medium"
                          value={aiPrompt}
                        ...
                        <button 
                          ...
                          className="px-5 py-3 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:hover:bg-indigo-600 text-white rounded-xl text-xs font-black uppercase tracking-widest transition flex items-center justify-center gap-1.5 shrink-0 cursor-pointer"
                        >
                          {aiLoading ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 bg-white/5 hover:bg-white/10 rounded-full text-[9px] font-bold text-slate-400 hover:text-white transition cursor-pointer border border-white/5"
                          >
                            + {suggestion}
                          </button>
                        ))}
                    ...
                    <div className="flex-1 relative">
                        ...
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-indigo-500/10 pb-3">
                            <div>
                              ...
                              <h5 className="text-sm font-black text-white tracking-tight uppercase mt-0.5">
                                {aiReportResult.reportTitle}
                              </h5>
                              <p className="text-[9px] font-bold text-slate-500 uppercase mt-0.5 tracking-wider">
                                Boundary Match: <span className="text-slate-300 font-black">{aiReportResult.scopedBoundary}</span> • Date Extracted: <span className="text-slate-300 font-black">{new Date().toLocaleDateString('en-GB')}</span>
                              </p>
                            </div>
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-black uppercase tracking-widest transition flex items-center gap-1.5 shrink-0 cursor-pointer border border-white/10"
                            >
                          ...
                          <div className="space-y-1.5 pt-1">
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Filtered Stats Preview ({aiReportResult.filteredModules?.length || 0} nodes)</p>
                            
                            <div className="max-h-[120px] overflow-y-auto border border-white/5 rounded-xl bg-black/40 text-[10px]">
                              {aiReportResult.filteredModules?.length > 0 ? (
                                <table className="w-full text-left font-sans text-slate-400">
                                  <thead className="bg-white/5 text-[9px] font-black text-slate-500 uppercase tracking-wider">
                                    <tr>
                                  ...
                                  <tbody className="divide-y divide-white/5 font-semibold">
                                    {aiReportResult.filteredModules.map((item, idx) => (
                                      <tr key={idx} className="hover:bg-white/[0.02]">
                                        <td className="p-2 font-black text-slate-300 tracking-tight">{item.code}</td>
                                ...
                                <div className="p-4 text-center text-slate-650 font-black uppercase text-[10px]">
                      ...
                      {!aiLoading && !aiError && !aiReportResult && (
                        <div className="h-full flex flex-col items-center justify-center p-8 border border-dashed border-white/5 rounded-2xl bg-white/[0.01]">
                          <Bot className="w-8 h-8 text-slate-600 mb-2 animate-bounce" style={{ fill: 'none' }} />
